...
thresholds have been chosen on train/validation data.
    ...
    .venv/bin/python scripts/compare_approaches.py --limit 10
"""
...
def load_sample(parquet_path: Path, limit: int) -> list[tuple[dict[str, Any], str]]:
    connection = duckdb.connect()
    try:
        return connection.execute(
            f"SELECT image, ground_truth FROM read_parquet(?) LIMIT {int(limit)}",
            [str(parquet_path)],
        ).fetchall()
    finally:
        connection.close()
...
def main() -> int:
    ...
    rows = load_sample(args.parquet, args.limit)
    print(f"Loaded {len(rows)} receipts from {args.parquet}")

    baseline_evals: list[DocumentEvaluation] = []
    gemini_evals: list[DocumentEvaluation] = []
    ...
    for index, (image_struct, ground_truth_json) in enumerate(rows):
        ...
        baseline_evals.append(evaluate_document(baseline_prediction, ground_truth))
        ...
        if gemini_prediction is not None:
            gemini_evals.append(evaluate_document(gemini_prediction, ground_truth))
        else:
    ...
    result = {
        ...
        "gemini_failures": sum(1 for entry in per_document if entry["gemini_error"]),
